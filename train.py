...
import random
import time
from utils.train_utils import select_bboxes_train, adjust_nums_of_points, augment_objs, displace_points, update_votes, get_pseudo_bboxess
...
# new
parser.add_argument('--augmentor_checkpoint', default=None)
...
parser.add_argument('--box_num_unlabeled', type=int, default=3, help='num of sample boxes')
...
detector_checkpoint = FLAGS.detector_checkpoint if FLAGS.detector_checkpoint is not None \
...
if detector_checkpoint is not None and os.path.isfile(detector_checkpoint):
    ...
    print("Loaded votenet checkpoint %s (epoch: %d)" % (detector_checkpoint, epoch_ckpt))

if FLAGS.augmentor_checkpoint is not None and os.path.isfile(FLAGS.augmentor_checkpoint):
    checkpoint = torch.load(FLAGS.augmentor_checkpoint)
    pretrained_dict = checkpoint['model_state_dict']

    ########
    model_dict = augmentor.state_dict()
    # 1. filter out unnecessary keys
    pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict}
    # 2. overwrite entries in the existing state dict
    model_dict.update(pretrained_dict)
    # 3. load the new state dict
    augmentor.load_state_dict(model_dict)
    ########

    # augmentor.load_state_dict(pretrained_dict)
    epoch_ckpt = checkpoint['epoch']
    print("Loaded augmentor checkpoint %s (epoch: %d)" % (FLAGS.augmentor_checkpoint, epoch_ckpt))
...
def train_one_epoch(global_step):
    ...
    for batch_idx, batch_data_label in enumerate(LABELED_DATALOADER):
        ...
        # get pseudo labels to select bboxs in unlabeled scenes
        ema_inputs = {'point_clouds': batch_data_label['ema_point_clouds']}
        with torch.no_grad():
            ema_end_points = ema_detector.forward_with_pred_jitter(ema_inputs)
        pred_bboxes = get_pseudo_bboxess(batch_data_label,
                                         ema_end_points,
                                         CONFIG_DICT,
                                         DATASET_CONFIG,
                                         FLAGS.box_num_unlabeled)
        ...
        # points: [box_size, OBJECT_POINTS_NUM, 3]
        if sel_bbox_nums > 1:
        ...
        aug_end_points = detector.forward_with_pred_jitter(aug_inputs)
        ...
        aug_detection_loss, aug_end_points = train_labeled_criterion(aug_end_points, DATASET_CONFIG, CONFIG_DICT)
        aug_unlabeled_loss, aug_end_points = train_unlabeled_criterion(aug_end_points, ema_end_points, DATASET_CONFIG, CONFIG_DICT)

        # weighted unlabeled loss
        WUL = (unlabeled_loss + aug_unlabeled_loss) * FLAGS.unlabeled_loss_weight
        loss = detection_loss + WUL + aug_detection_loss
