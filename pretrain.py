...
import random
from utils.train_utils import select_bboxes_pretrain, adjust_nums_of_points, augment_objs, displace_points, update_votes, cal_aug_loss
#from scipy.spatial import Delaunay

parser = argparse.ArgumentParser()
parser.add_argument('--dataset', default='scannet', help='Dataset name.[default: scannet]')
parser.add_argument('--checkpoint_path', default=None, help='Model checkpoint path [default: None]')
parser.add_argument('--aug_checkpoint_path', default=None, help='Model checkpoint path [default: None]')
...
parser.add_argument('--warm_up', type=int, default=0, help='num of epoch to warm up augmentor')
...
# new
AUGMENTOR_LEARNING_RATE = FLAGS.learning_rate_a
OBJECT_POINTS_NUM = FLAGS.obj_points_num
...
DEFAULT_AUG_CHECKPOINT_PATH = os.path.join(LOG_DIR, 'aug_checkpoint.tar')
AUG_CHECKPOINT_PATH = FLAGS.aug_checkpoint_path if FLAGS.aug_checkpoint_path is not None \
    else DEFAULT_AUG_CHECKPOINT_PATH
...
optimizer_a = torch.optim.Adam(
            ...
            lr=AUGMENTOR_LEARNING_RATE,
            betas=(0.9, 0.999),
            eps=1e-08,
            weight_decay=FLAGS.decay_rate
        )
...
if CHECKPOINT_PATH is not None and os.path.isfile(CHECKPOINT_PATH):
    ...
    aug_checkpoint = torch.load(AUG_CHECKPOINT_PATH)
    augmentor.load_state_dict(aug_checkpoint['model_state_dict'])

    if FLAGS.resume:
        optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
        optimizer_a.load_state_dict(aug_checkpoint['optimizer_state_dict'])
        start_epoch = checkpoint['epoch']
    log_string("-> loaded checkpoint %s (epoch: %d)" % (CHECKPOINT_PATH, start_epoch))
    log_string("-> loaded checkpoint %s (epoch: %d)" % (AUG_CHECKPOINT_PATH, start_epoch))
...
def train_one_epoch(epoch):
    ...
    for batch_idx, batch_data_label in enumerate(TRAIN_DATALOADER):
        ...
        optimizer.zero_grad()
        aug_batch_data_label = {}
        for key in batch_data_label:
            aug_batch_data_label[key] = batch_data_label[key].detach().clone()

        # start to prepare bboxes to augment
        # select random bboxes to augment
        batch_obj_index, selected_bbox, sel_bbox_nums = select_bboxes_pretrain(batch_data_label, FLAGS.dataset, FLAGS.box_num)

        # points: [box_size, max_pc_size, 3]
        if sel_bbox_nums > 1:
            # Adjust the numbers of points in the bbox to the assign number
            points, near_points_list = adjust_nums_of_points(batch_data_label, batch_obj_index, OBJECT_POINTS_NUM)

            # Augment selected objects
            displacement = augment_objs(augmentor, points, OBJECT_POINTS_NUM)

            # add displacement to points
            displace_points(batch_data_label,
                            aug_batch_data_label, 
                            batch_obj_index, 
                            selected_bbox,
                            near_points_list,
            ...
            # update point vote
            labeled_num = aug_batch_data_label['point_clouds'].size()[0]
            update_votes(FLAGS.dataset, aug_batch_data_label, DATASET_CONFIG, FLAGS.num_point, labeled_num)
        else:
            print('batch bbox num < 2')

        # Forward pass
        inputs = {'point_clouds': batch_data_label['point_clouds']}
        aug_inputs = {'point_clouds': aug_batch_data_label['point_clouds']}

        end_points = net.forward_with_pred_jitter(inputs)
        aug_end_points = net.forward_with_pred_jitter(aug_inputs)

        # Compute loss and gradients, update parameters.
        for key in batch_data_label:
            assert (key not in end_points)
            end_points[key] = batch_data_label[key]
        for key in aug_batch_data_label:
            assert (key not in aug_end_points)
            aug_end_points[key] = aug_batch_data_label[key]

        loss, end_points = get_labeled_loss(end_points, DATASET_CONFIG, CONFIG_DICT)
        augLoss, aug_end_points = get_labeled_loss(aug_end_points, DATASET_CONFIG, CONFIG_DICT)
        
        AugLoss = cal_aug_loss(end_points,
                               loss,
                               augLoss,
                               FLAGS.Lambda,
                               epoch,
...
def train(start_epoch):
    ...
    global BEST_MAP
    EPOCH_CNT = 0
    DEC_loss_list, AUG_loss_list, RIGHT_loss_list, epochs_list = [], [], [], []
    ...
    for epoch in range(start_epoch, MAX_EPOCH):
        ...
        if EPOCH_CNT % FLAGS.eval_interval == 0 and EPOCH_CNT > 0:
            ...
            if map[0] + map[1] > BEST_MAP[0] + BEST_MAP[1]:
                ...
                torch.save(save_dict, os.path.join(LOG_DIR, 'best_checkpoint_sum_{}.tar'.format(EPOCH_CNT)))

                save_aug_dict = {'epoch': epoch + 1,  # after training one epoch, the start_epoch should be epoch+1
                             'optimizer_state_dict': optimizer_a.state_dict(),
                             'loss': loss
                             }
                try:  # with nn.DataParallel() the net is added as a submodule of DataParallel
                    save_aug_dict['model_state_dict'] = augmentor.module.state_dict()
                except:
                    save_aug_dict['model_state_dict'] = augmentor.state_dict()
                torch.save(save_aug_dict, os.path.join(LOG_DIR, 'aug_best_checkpoint_sum_{}.tar'.format(EPOCH_CNT)))
        ...
        try:  # with nn.DataParallel() the net is added as a submodule of DataParallel
            ...
            save_aug_dict['model_state_dict'] = augmentor.state_dict()
        torch.save(save_aug_dict, os.path.join(LOG_DIR, 'aug_checkpoint.tar'))
        ...
        if EPOCH_CNT % FLAGS.save_interval == 0:
            ...
            try:  # with nn.DataParallel() the net is added as a submodule of DataParallel
                ...
                save_aug_dict['model_state_dict'] = augmentor.state_dict()
            torch.save(save_aug_dict, os.path.join(LOG_DIR, 'aug_checkpoint_%d.tar' % EPOCH_CNT))
