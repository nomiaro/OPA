...
'''
def select_bboxes(batch_data_label, dataset, box_num):
    ...
    for data_label_index in range(batch_size):
        ...
        for bbox_ind in bbox_selected:
            ...
            pc = batch_data_label['point_clouds'][data_label_index].cpu().detach().numpy()
...
def select_bboxes_train(batch_data_label, dataset, box_num, box_num_unlableed, pred_bboxes, labeled_num):
    ...
    for data_label_index in range(batch_size):
        # labeled
        if data_label_index < labeled_num:
            bbox_size = batch_data_label['instance_bboxes_size'][data_label_index]
            if bbox_size < 1:
                print('bbox size < 1')
                batch_obj_index.append(None)
        ...
        # unlabled
            ...
            bbox_size = len(pred_bboxes['center'][data_label_index-labeled_num])
            if bbox_size < 1:
                print('pseudo bbox size < 1')
                batch_obj_index.append(None)
                selected_bbox['center'].append(None)
            ...
            if bbox_size <= box_num_unlableed:
                bbox_selected = [i for i in range(bbox_size)]
            else:
                bbox_selected = random.sample(range(bbox_size), k=box_num_unlableed)
            ...
            size_list = []
            for bbox_ind in bbox_selected:
                center = pred_bboxes['center'][data_label_index-labeled_num][bbox_ind]
                if center.shape[0] < 1:
                    continue
                size = pred_bboxes['size'][data_label_index-labeled_num][bbox_ind]
                #size /= 2.0
                box_3d_corners = my_compute_box_3d(center, size)
                pc = batch_data_label['point_clouds'][data_label_index].cpu().detach().numpy()
                try:
