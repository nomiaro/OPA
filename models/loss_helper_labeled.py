...
def compute_box_and_sem_cls_loss(end_points, supervised_inds, dataset_config, config_dict):
    ...
    bbox_conf = F.softmax(end_points['sem_cls_scores'][supervised_inds, ...], dim=-1)
    one_hot = F.one_hot(sem_cls_label, bbox_conf.size()[-1]).float()
    # obj_score
    pred_objectness = end_points['objectness_scores'][supervised_inds, ...]
    pred_objectness = nn.Softmax(dim=2)(pred_objectness)
    pos_obj_scores = pred_objectness[:, :, 1]
    # bbox confidence
    end_points['bbox_conf'] = torch.sum(
        ((bbox_conf * one_hot).max(dim=-1)[0] * objectness_label)) / (
            torch.sum(objectness_label) + 1e-6)
