...
class ScannetSSLLabeledDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #point_color = point_cloud[:,3:6]
        #point_cloud = np.concatenate([point_cloud[:,:3], np.expand_dims(point_cloud[:,6], 1)], 1)
        #ema_point_cloud = np.concatenate([ema_point_cloud[:,:3], np.expand_dims(ema_point_cloud[:,6], 1)], 1)
        ...
        ret_dict['point_colors'] = point_color.astype(np.float32)
        '''
        ...
        # new
        ret_dict['instance_bboxes_size'] = instance_bboxes.shape[0]
        ret_dict['bbox'] = target_bboxes.astype(np.float32)
